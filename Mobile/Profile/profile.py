...
class UserProfile(ft.UserControl):
    ...
    def edit_profile(self, e):
        ...
        show_dialog(self.page, 'Editează profil-ul', ft.Column([
    ...
    def update_user_profile(self, data):
        try:
            ...
            show_snackbar(self.page, 'Profilul a fost actualizat', 'green')
            self.close_dialog()
            self.get_info_user()
            self.page.update()
        except requests.exceptions.RequestException as e:
            show_snackbar(self.page, f'Error: {e}', 'red')
    ...
    def change_password(self, e):
        ...
        show_dialog(self.page, 'Schimbă parola', ft.Column([
    ...
    def update_password(self, current_password, new_password):
        if len(new_password) < 6:
            show_snackbar(self.page, 'Parola trebuie să fie de cel puțin 6 caractere', 'red')
            return
        if new_password == current_password:
            show_snackbar(self.page, 'Noua parolă nu poate fi aceeași cu parola actuală', 'red')
            return

        data = {'password': current_password, 'new_password': new_password}
        response = self.api_client.change_password(self.token, data=data)
        if response.status_code == 200:
            show_snackbar(self.page, "Parola a fost schimbată", 'green')
        else:
